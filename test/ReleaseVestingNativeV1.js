...
describe("ReleaseVestingNativeV1 Contract", function () {
  ...
  async function deployContractFixture() {
    ...
    const vesting = await hre.ethers.deployContract("ReleaseVestingNativeV1", [360]);
    await vesting.waitForDeployment();

    return { vesting, owner, otherAccount, unlockTime };
  }

  describe("ReleaseVestingContract Events", function () {
    it("Should emit an event on release", async function () {
      const { vesting, owner, otherAccount } = await loadFixture(
      ...
      await expect(vesting.connect(otherAccount).createVestingSchedule(otherAccount.address, startTime, duration, durUnit, amount, { value: amount }))
  ...
  describe("ReleaseVestingContract CreateVestingSchedule and Release to staker", function () {
    it("Should release the funds to the staker", async function () {
      const { vesting, owner, otherAccount, unlockTime, scheduleRewards } = await loadFixture(
        deployContractFixture
      );
      ...
      userTokenBalanceBefore = await hre.ethers.provider.getBalance(otherAccount);
      console.log("userTokenBalanceBefore:\t%d Ether", ethers.formatEther(userTokenBalanceBefore));

      // createVestingSchedule 2 times
      await vesting.connect(otherAccount).createVestingSchedule(otherAccount.address, startTime, duration, durUnit, ethers.parseEther("50"), { value: ethers.parseEther("50") });
      await vesting.connect(otherAccount).createVestingSchedule(otherAccount.address, startTime, duration, durUnit, ethers.parseEther("50"), { value: ethers.parseEther("50") });
      ...
      userTokenBalanceAfter = await hre.ethers.provider.getBalance(otherAccount);
      ...
      contractTokenBalance = await hre.ethers.provider.getBalance(vesting);
      ...
      userTokenBalanceAfter = await hre.ethers.provider.getBalance(otherAccount.address);
    ...
    it("Should release the funds to the staker after 360 days", async function () {
      ...
      await expect(vesting.connect(otherAccount).createVestingSchedule(otherAccount.address, now, duration, durUnit, ethers.parseEther("100"), { value: ethers.parseEther("100") }))
        .to.be.revertedWith("VestingContract: invalid start time");
      await expect(vesting.connect(otherAccount).createVestingSchedule(otherAccount.address, startTime - 120, duration, durUnit, ethers.parseEther("100"), { value: ethers.parseEther("100") }))
        .to.be.revertedWith("VestingContract: invalid start time");
